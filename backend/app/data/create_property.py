...
def parse_decimal(value):
    return float(value)
    # Convert value to string, replace commas with nothing and periods with decimal points, then convert to float
    # try:
    #     return float(str(value).replace(",", "").replace(".", ".")) if value and str(value).lower() != 'nan' else None
    # except ValueError:
    #     return None

def parse_integer(value):
    # Convert value to string, remove commas and periods, then convert to int
    try:
        return int(value)
    except:
        return None
    # try:
    #     return int(str(value).replace(",", "").replace(".", "")) if value and str(value).lower() != 'nan' else None
    # except ValueError:
    #     return None

def parse_date(value):
    # Ensure date is in YYYY-MM-DD format or adjust the strptime format accordingly
    try:
